import { useEffect, useState } from "react"
import { encryptedLocalStorage } from "../utils/secureLocalStorage"


const decode = (value:any) => {
  return JSON.stringify(value)
}

const encode = (value:any) => {
  return JSON.parse(value)
}

const useLocalStorage = (key:any, defaultState:any) => {
  const [value, setValue] = useState(
    encode(localStorage.getItem(key)||null) || defaultState
  )

  useEffect(() => {
    localStorage.setItem(key, decode(value))
  },  [value])

  return [value, setValue]
}

const useSecureLocalStorage = (key:any, defaultState:any) => {
  console.log('useSecureLocalStorage', key, decode(defaultState))
  encryptedLocalStorage.setItem(key, decode(defaultState))
  //console.log('get enc',encryptedLocalStorage.getItem(key))
 /*  const [value, setValue] = useState(
    encode(encryptedLocalStorage.getItem(key)||null) || defaultState
  ) */

 /*  useEffect(() => {
    console.log('useSecureLocalStorage- useEffect', key, defaultState, value)
    encryptedLocalStorage.setItem(key, decode(value))
  },  [value]) */

  //return [value, setValue]
}


export {
  useLocalStorage,
  useSecureLocalStorage
}